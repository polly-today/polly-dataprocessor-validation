...
def compare_llm_to_target_output(input, response):
    ...
    print("Checking required columns and preprocessing data...")
    ...
    print("LLM supplier name:", supplier_name)
    print("Target supplier name:", target_output_df["supplier_name"].unique())
    # Check if the supplier_name in the LLM output matches the target output
    if supplier_name not in target_output_df["supplier_name"].unique():
        raise ValueError(f"Supplier name '{supplier_name}' not found in target output.")
    if date_of_sending not in target_output_df["date_of_sending"].unique():
        raise ValueError(f"Date of sending '{date_of_sending}' not found in target output.")
    if email_adress not in target_output_df["email_address"].unique():
        raise ValueError(f"Email address '{email_adress}' not found in target output.")
    # if phone_number not in target_output_df["phone_number"].unique():
    #     raise ValueError(f"Phone number '{phone_number}' not found in target output.")
    if email_subject not in target_output_df["email_subject"].unique():
        raise ValueError(f"Email subject '{email_subject}' not found in target output.")
...
inputs = load_inputs()
input_id = 6
input = inputs[inputs["id"] == input_id]
response = """
...
"""
value_comparison_df = compare_llm_to_target_output(input, response)